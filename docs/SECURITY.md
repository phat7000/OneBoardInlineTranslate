...
- Untrusted transport: HTTPS required for remote provider endpoints; HTTP allowed only for loopback LibreTranslate.
...
The app has no backend and performs no translation network request before a user configures a provider and invokes an operation. Provider requests use supported JSON/form APIs and a bounded timeout. Unofficial Google Translate endpoints and scraping are prohibited.
...
Version 1.0.0 is unsigned unless the GitHub release explicitly states otherwise. No self-signed certificate is presented as trusted. Windows SmartScreen may warn when the executable or installer is first downloaded.
